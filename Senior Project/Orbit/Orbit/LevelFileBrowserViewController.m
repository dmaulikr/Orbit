...
- (void) addAction: (id) sender {
    ...
    [self presentModalViewController:lffvc animated: YES];
    [lffvc release];
}
...
- (void)viewDidLoad
{
    ...
    if (self.delegate) {
        if ([(NSObject*)self.delegate isKindOfClass: [LevelEditorViewController class]]) {
            ...
            self.navigationItem.rightBarButtonItem = [[[UIBarButtonItem alloc] initWithBarButtonSystemItem: UIBarButtonSystemItemCancel target: self action: @selector(quitAction:)] autorelease];
        }
    }
}
